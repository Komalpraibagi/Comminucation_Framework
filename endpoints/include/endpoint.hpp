...
class endpoint {
public:
    typedef void (*error_handler_t)();      // crevavi changed
    typedef void (*prepare_stop_handler_t)(const std::shared_ptr<endpoint>&);        // crevavi changed
    ...
    virtual unsigned long get_queue_size() const = 0;          // crevavi size_t to unsigned long
