...
/*
Feature boost::icl::interval_set                                     lwIP
Purpose Generic interval arithmetic (e.g., [1,10] ∪ [5,15])          Lightweight TCP/IP stack (no interval logic)
Use Case    Scheduling, time ranges, numeric intervals               Network packet handling, sockets
Language    C++ (templates, STL)                                     C (no STL/containers)
*/
#include <boost/icl/interval_set.hpp>       //purposegeneric interval arithmatic
...
public:
    ...
    virtual const ip_addr_t & get_unicast_address() const = 0;
    virtual const ip_addr_t& get_netmask() const = 0;
    ...
    virtual const ip_addr_t &get_routing_host_address() const = 0;
    virtual port_t get_routing_host_port() const = 0;

    virtual const ip_addr_t &get_routing_guest_address() const = 0;
    ...
    virtual unsigned long get_max_dispatchers(const std::string &_name) const = 0;
    virtual unsigned long get_max_dispatch_time(const std::string &_name) const = 0;
    virtual unsigned long get_max_detached_thread_wait_time(const std::string& _name) const = 0;
    virtual unsigned long get_io_thread_count(const std::string &_name) const = 0;
    virtual int get_io_thread_nice_level(const std::string &_name) const = 0;
    virtual unsigned long get_request_debouncing(const std::string &_name) const = 0;
    ...
    virtual bool supports_selective_broadcasts(const ip_addr_t &_address) const = 0;
    ...
    virtual bool is_protected_device(
            const ip_addr_t & _address) const = 0;
    virtual bool is_protected_port(
            const ip_addr_t & _address, std::uint16_t _port,
    ...
    virtual void set_sd_acceptance_rules_active(
            const ip_addr_t & _address, bool _enable) = 0;
    ...
    virtual reliability_type_e get_reliability_type(
            const ip_addr_t &_reliable_address,
            const uint16_t &_reliable_port,
            const ip_addr_t &_unreliable_address,
